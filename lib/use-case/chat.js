...
var TypeVector = require('@goodmind/telegram-tl-node').TypeVector;
...
function chat(peer) {
    return new Promise(function (fulfill, reject) {
        open(peer).then(function () {
            /*ui.askWordInput('Message', '').then(function (msg) {
                clientProxy.getClient().messages.sendMessage(
                    ...
                    null,
                    null
                ).then(sent => console.log(sent)).then(fulfill, reject);
            });*/
            fulfill();
        }, reject);
    });
        ...
        try {
            clientProxy.getClient().messages.getHistory(peer, 0, 0, OPEN_TOTAL_MESSAGES).then(function (container) {
                try {
                    ...
                    // title
                    var title;
                    if (peer.instanceOf('api.type.InputPeerChat') || peer.instanceOf('api.type.InputPeerChannel')) {
                        title = container.chats.getById(peer.chat_id || peer.channel_id).title;
                    } else {
