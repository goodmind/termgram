...
function makePasswordHash (salt, password) {
  ...
  return mt.utility.createSHAHash(buffer, 'sha256')
}

function ask2FAPassword(auth) {
    console.log(auth)
    if (auth.res.current_salt) {
        return ui.askPasswordInput('Password').then(function (password) {
            const hash = makePasswordHash(auth.res.current_salt, password)
            console.log(hash)
            return clientProxy.checkPassword(hash).then(function(res) {
                console.log(res)
                return {
                    code: auth.code,
                    res: res
            ...
            askPassword(function (password) {
                ...
                clientPromise.then(function (dataCenter) {
                    ...
                    askPhoneNumberToSendCode().then(function (res) {
                        ...
                        askAuthCode(res.phoneNumber, res.sendCodeRes.phone_code_hash).then(ask2FAPassword).then(function (authCodeRes) {
