...
var api = require('@goodmind/telegram.link')();
...
function selectChat() {
    return new Promise(function (fulfill, reject) {
        ui.spinner();
        try {
            clientProxy.getClient().messages.getDialogs(0, 0, new api.type.InputPeerEmpty(), 0).then(function (container) {
                try {
                    ui.spinner();
...
function renderChat(container, index) {
    ...
    var msgText = (!message.media || message.media.instanceOf('api.type.MessageMediaEmpty')) ? message.message || '[none]' : '[image]';
    var maxLength = 50;
    ...
    var inputPeer;
    if (peer.instanceOf('api.type.PeerChannel')) {
        if (message.from_id) {
          var chat = container.chats.getById(peer.channel_id);
          title = chat.title;
          user = container.users.getById(message.from_id);
          inputPeer = new api.type.InputPeerChannel();
          inputPeer.channel_id = peer.channel_id;
          inputPeer.access_hash = chat.access_hash;
        }
    } else if (peer.instanceOf('api.type.PeerChat')) {
...
function retrieveInputPeer(user) {
    var inputPeer;
    //console.log(user, api.type.InputPeerUser.toString())
    if (user.instanceOf('api.type.User')) {
        inputPeer = new api.type.InputPeerUser();
